...
async function handleSearch(e) {
  ...
  const firefoxResults = await BookmarkUtils.search(query);
  ...
  // 先添加 Firefox API 结果（过滤当前目录）
  firefoxResults.filter(n => n.url && folderIds.includes(n.parentId)).forEach(bookmark => {
  ...
  // 过滤文件夹
  const folders = firefoxResults.filter(n => !n.url && folderIds.includes(n.id));
...
function setupEventListeners() {
  ...
  // 监听 Firefox 书签变化，实时更新界面
  browser.bookmarks.onCreated.addListener(handleBookmarkChanged);
  browser.bookmarks.onRemoved.addListener(handleBookmarkChanged);
  browser.bookmarks.onChanged.addListener(handleBookmarkChanged);
  browser.bookmarks.onMoved.addListener(handleBookmarkChanged);
}
