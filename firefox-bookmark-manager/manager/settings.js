...
async function saveWebDAVConfig() {
  ...
  await browser.runtime.sendMessage({ action: 'init' });
...
async function exportAllConfig() {
  try {
    ...
    // 导出标签
    if (exportTags) {
      const tagsResult = await browser.storage.local.get('bookmark_tags');
...
async function handleImportAllConfig(event) {
  ...
  try {
    ...
    // 导入标签
    if (data.tags) {
      const existingTags = await browser.storage.local.get('bookmark_tags');
      const mergedTags = { ...(existingTags.bookmark_tags || {}), ...data.tags };
      await browser.storage.local.set({ bookmark_tags: mergedTags });
    }
...
function handleImportTagsFile(event) {
  ...
  reader.onload = async (e) => {
    try {
      ...
      // 合并标签数据
      const existing = await browser.storage.local.get('bookmark_tags');
      const merged = {
...
async function refreshBackupList() {
  ...
  try {
    const response = await browser.runtime.sendMessage({ action: 'listBackups' });
