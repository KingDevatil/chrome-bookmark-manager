...
const BookmarkUtils = {
  ...
  getTree() {
    return new Promise((resolve, reject) => {
      ExtensionAPI.bookmarks.getTree((tree) => {
        if (ExtensionAPI.runtime.lastError) {
          reject(ExtensionAPI.runtime.lastError);
        } else {
          resolve(tree);
  ...
  getChildren(parentId) {
    return new Promise((resolve, reject) => {
      ExtensionAPI.bookmarks.getChildren(parentId, (children) => {
        if (ExtensionAPI.runtime.lastError) {
          reject(ExtensionAPI.runtime.lastError);
        } else {
          resolve(children);
  ...
  search(query) {
    return new Promise((resolve, reject) => {
      ExtensionAPI.bookmarks.search(query, (results) => {
        if (ExtensionAPI.runtime.lastError) {
          reject(ExtensionAPI.runtime.lastError);
        } else {
          resolve(results);
  ...
  create(bookmark) {
    return new Promise((resolve, reject) => {
      ExtensionAPI.bookmarks.create(bookmark, (result) => {
        if (ExtensionAPI.runtime.lastError) {
          reject(ExtensionAPI.runtime.lastError);
        } else {
          resolve(result);
  ...
  update(id, changes) {
    return new Promise((resolve, reject) => {
      ExtensionAPI.bookmarks.update(id, changes, (result) => {
        if (ExtensionAPI.runtime.lastError) {
          reject(ExtensionAPI.runtime.lastError);
        } else {
          resolve(result);
  ...
  move(id, destination) {
    return new Promise((resolve, reject) => {
      ExtensionAPI.bookmarks.move(id, destination, (result) => {
        if (ExtensionAPI.runtime.lastError) {
          reject(ExtensionAPI.runtime.lastError);
        } else {
          resolve(result);
  ...
  remove(id) {
    return new Promise((resolve, reject) => {
      ExtensionAPI.bookmarks.remove(id, () => {
        if (ExtensionAPI.runtime.lastError) {
          reject(ExtensionAPI.runtime.lastError);
        } else {
          resolve();
  ...
  removeTree(id) {
    return new Promise((resolve, reject) => {
      ExtensionAPI.bookmarks.removeTree(id, () => {
        if (ExtensionAPI.runtime.lastError) {
          reject(ExtensionAPI.runtime.lastError);
        } else {
          resolve();
  ...
  async getPath(id) {
    ...
    while (currentId) {
      const node = await new Promise((resolve, reject) => {
        ExtensionAPI.bookmarks.get(currentId, (results) => {
          if (ExtensionAPI.runtime.lastError) {
            reject(ExtensionAPI.runtime.lastError);
          } else {
            resolve(results[0]);
...
const ShortcutUtils = {
  ...
  add(...args) { return domainCommand('shortcuts','add',args); },

  remove(...args) { return domainCommand('shortcuts','remove',args); },

  update(...args) { return domainCommand('shortcuts','update',args); },

  reorder(...args) { return domainCommand('shortcuts','reorder',args); },

  clear(...args) { return domainCommand('shortcuts','clear',args); },

  async import(shortcuts) {
    return domainCommand('shortcuts', 'import', [shortcuts]);
  }
};
...
const BookmarkTags = {
  ...
  setTags(...args) { return domainCommand('tags','setTags',args); },

  addTag(...args) { return domainCommand('tags','addTag',args); },

  removeTag(...args) { return domainCommand('tags','removeTag',args); },

  removeTags(...args) { return domainCommand('tags','removeTags',args); },
  ...
  async detectOrphanedTags() {
    ...
    for (const [bookmarkId, tags] of Object.entries(allTags)) {
      const bookmarks = await new Promise((resolve) => {
        ExtensionAPI.bookmarks.get(bookmarkId, (results) => {
          resolve(results);
        });
      });
  ...
  cleanOrphanedTags(...args) { return domainCommand('tags','cleanOrphanedTags',args); }
};
...
const TagGroups = {
   ...
   */
  save(...args) { return domainCommand('groups','save',args); },
   ...
   */
  createGroup(...args) { return domainCommand('groups','createGroup',args); },
  
  /**
   * 删除分组
   * @param {string} groupId - 分组ID
   */
  deleteGroup(...args) { return domainCommand('groups','deleteGroup',args); },
   ...
   */
  renameGroup(...args) { return domainCommand('groups','renameGroup',args); },
   ...
   * @param {string} tag - 标签名
   */
  addTagToGroup(...args) { return domainCommand('groups','addTagToGroup',args); },
   ...
   * @param {string} tag - 标签名
   */
  removeTagFromGroup(...args) { return domainCommand('groups','removeTagFromGroup',args); },
   ...
   * 清空所有分组
   */
  clearAll(...args) { return domainCommand('groups','clearAll',args); }
};

async function domainCommand(domain, method, args) {
 const result=await ExtensionAPI.runtime.sendMessage({action:'domain',domain,method,args});
 if(!result?.success) throw new Error(result?.error || '操作失败'); return result.data;
}
BookmarkTags.batchAdd=(ids,tags)=>domainCommand('tags','batchAdd',[ids,tags]);
BookmarkTags.detectOrphanedTags=async()=>{const tags=await BookmarkTags.getAll();const live=new Set(BookmarkUtils.flatten(await BookmarkUtils.getTree()).map(n=>n.id));return Object.fromEntries(Object.entries(tags).filter(([id])=>!live.has(id)));};
let treeCache, treePending, treeRevision=0;
BookmarkUtils.getTree=()=>{
 if(treeCache) return Promise.resolve(treeCache);
 if(treePending) return treePending;
 const revision=treeRevision;
 const pending=ExtensionAPI.bookmarks.getTree().then(tree=>{if(revision===treeRevision)treeCache=tree;return tree;}).finally(()=>{if(treePending===pending)treePending=null;});
 treePending=pending;return pending;
};
for(const event of ['onCreated','onChanged','onMoved','onRemoved','onChildrenReordered','onImportEnded']) ExtensionAPI.bookmarks[event]?.addListener(()=>{treeRevision++;treeCache=null;treePending=null;});
