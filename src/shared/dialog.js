...
class Dialog {
  ...
  static show(options) {
    this.init();
    if(this.currentResolve)this.close(false);
    ...
    if (type === 'prompt') {
      body.innerHTML = `
        <div style="margin-bottom: 12px;">${Utils.escapeHtml(String(message))}</div>
        <input type="text" id="dialog-input" class="dialog-input" value="${Utils.escapeHtml(String(inputValue))}"
