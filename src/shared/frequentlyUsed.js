...
const FrequentlyUsed = {
  ...
  async calculateFrequentlyUsed(daysRange, displayCount, blacklist, pinned = []) {
    ...
    try {
      ...
      const history = await ExtensionAPI.history.search({
      ...
      const countMap = {};
      await mapConcurrent(filtered, 6, async (item) => {
        // 获取该 URL 的详细访问记录
        const visits = await ExtensionAPI.history.getVisits({ url: item.url });
  ...
  async isBookmarked(url) {
    try {
      const results = await ExtensionAPI.bookmarks.search({ url });
...
async function mapConcurrent(items,limit,fn){let cursor=0;await Promise.all(Array.from({length:Math.min(limit,items.length)},async()=>{while(cursor<items.length){const index=cursor++;await fn(items[index],index);}}));}
const pendingFrequent=new Map();const getFrequent=FrequentlyUsed.getFrequentlyUsed.bind(FrequentlyUsed);
FrequentlyUsed.getFrequentlyUsed=(...args)=>{const key=JSON.stringify(args);if(!pendingFrequent.has(key))pendingFrequent.set(key,getFrequent(...args).finally(()=>pendingFrequent.delete(key)));return pendingFrequent.get(key);};
ExtensionAPI.history.onVisited?.addListener(()=>FrequentlyUsed.clearCache());
ExtensionAPI.history.onVisitRemoved?.addListener(()=>FrequentlyUsed.clearCache());
