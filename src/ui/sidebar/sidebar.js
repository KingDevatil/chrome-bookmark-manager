...
let bookmarkTags = {};
let expandedFolders = new Set([PlatformRoots.toolbar, PlatformRoots.other]);
...
async function loadBookmarkTree() {
  try {
    ...
    renderBookmarkTree();
    loadFrequentlyUsedData().then(() => renderBookmarkTree()).catch(console.error);
  } catch (error) {
...
async function loadFrequentlyUsedData() {
  const revision=++frequentRevision;
  try {
    ...
    const data = await FrequentlyUsed.getFrequentlyUsed(
      config.daysRange,
      config.displayCount,
      config.blacklist,
      config.pinned || []
    );
    if(revision===frequentRevision)frequentlyUsedData=data;
  } catch (error) {
...
async function showFrequentlyUsedContextMenu(e) {
  ...
  configItem.addEventListener('click', () => {
    ExtensionAPI.tabs.create({
      url: ExtensionAPI.runtime.getURL('manager/settings.html')
    });
    removeContextMenu();
  });
...
function buildFolderOptions(nodes, level) {
  ...
  for (const node of nodes) {
    ...
    if (isFolder) {
      ...
      options += `<option value="${Utils.escapeHtml(node.id)}" ${selected}>${indent}${prefix}${Utils.escapeHtml(node.title || '新建文件夹')}</option>`;
...
async function showAddBookmarkModal(item = null) {
  removeEditModal();

  // 获取当前标签页信息
  let currentTab = item;
  try {
    const tabs = item ? [item] : await ExtensionAPI.tabs.query({ active: true, currentWindow: true });
  ...
  modal.innerHTML = `
    <div class="edit-modal-content">
      <h3 class="edit-modal-title">${Utils.escapeHtml(String(I18n.t('common.addBookmark')))}</h3>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.name')))}</label>
        <input type="text" class="edit-modal-input" id="add-title-input" value="${Utils.escapeHtml(String(currentTab?.title || ''))}">
      </div>
      <div class="edit-modal-field">
        <label class="edit-modal-label">URL</label>
        <input type="text" class="edit-modal-input" id="add-url-input" value="${Utils.escapeHtml(String(currentTab?.url || ''))}">
      </div>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.folder')))}</label>
        <div class="folder-tree" id="add-folder-tree"></div>
        <input type="hidden" id="add-folder-parentId" value="${Utils.escapeHtml(String(currentTab?.parentId || PlatformRoots.toolbar))}">
      </div>
      <div class="edit-modal-buttons">
        <button class="edit-modal-btn edit-modal-cancel">${Utils.escapeHtml(String(I18n.t('common.cancel')))}</button>
        <button class="edit-modal-btn edit-modal-save">${Utils.escapeHtml(String(I18n.t('common.save')))}</button>
      </div>
    </div>
  `;
  ...
  if (bookmarkTree && bookmarkTree.length > 0 && bookmarkTree[0].children) {
    ...
    for (const root of bookmarkTree[0].children) {
      ...
      if (isFolder) {
        const li = createFolderTreeNode(root, 0, PlatformRoots.toolbar);
...
async function showCreateFolderModal() {
  ...
  modal.innerHTML = `
    <div class="edit-modal-content">
      <h3 class="edit-modal-title">${Utils.escapeHtml(String(I18n.t('common.newFolderTitle')))}</h3>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.folderName')))}</label>
        <input type="text" class="edit-modal-input" id="new-folder-title" placeholder="${Utils.escapeHtml(String(I18n.t('common.enterFolderName')))}">
      </div>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.storageLocation')))}</label>
      ...
      <div class="edit-modal-buttons">
        <button class="edit-modal-btn edit-modal-cancel">${Utils.escapeHtml(String(I18n.t('common.cancel')))}</button>
        <button class="edit-modal-btn edit-modal-save">${Utils.escapeHtml(String(I18n.t('common.create')))}</button>
      </div>
    </div>
  `;
...
// 显示右键菜单
async function showContextMenu(e, node, isFolder) {
  // 移除已存在的菜单
  removeContextMenu();
  ...
  if (!isFolder && node.url) {
    ...
    addToShortcutsItem.addEventListener('click', async () => {
      try {
        await ShortcutUtils.add(node.title, node.url);
      } catch (error) {
        showToast('该网址已存在', 'warning');
      }
      removeContextMenu();
    ...
    const existsInShortcuts = await ShortcutUtils.exists(node.url);
    if (!existsInShortcuts) {
      menu.appendChild(addToShortcutsItem);
    }
  }
...
async function showEditModal(node) {
  ...
  modal.innerHTML = `
    <div class="edit-modal-content">
      <h3 class="edit-modal-title">${Utils.escapeHtml(String(I18n.t('common.editBookmark')))}</h3>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.name')))}</label>
        <input type="text" class="edit-modal-input" id="edit-title-input" value="${Utils.escapeHtml(String(node.title || ''))}">
      </div>
      <div class="edit-modal-field">
        <label class="edit-modal-label">URL</label>
        <input type="text" class="edit-modal-input" id="edit-url-input" value="${Utils.escapeHtml(String(node.url || ''))}">
      </div>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.folder')))}</label>
        ...
        <input type="hidden" id="edit-folder-parentId" value="${Utils.escapeHtml(String(node.parentId || PlatformRoots.toolbar))}">
      </div>
      <div class="edit-modal-buttons">
        <button class="edit-modal-btn edit-modal-cancel">${Utils.escapeHtml(String(I18n.t('common.cancel')))}</button>
        <button class="edit-modal-btn edit-modal-save">${Utils.escapeHtml(String(I18n.t('common.save')))}</button>
  ...
  modal.querySelector('.edit-modal-save').addEventListener('click', async () => {
    ...
    try {
      ...
      if (newParentId && newParentId !== node.parentId) {
        await ExtensionAPI.bookmarks.move(node.id, { parentId: newParentId });
      }
  ...
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div class="confirm-dialog">
        <div class="confirm-message">${Utils.escapeHtml(String(message))}</div>
...
async function handleDrop(e) {
  ...
  try {
    ...
    if (finalDropZone === 'inside' && isTargetFolder) {
      ...
      // 如果目标是根节点，直接移入根节点内部
      if (!parent && (targetNode.id === PlatformRoots.toolbar || targetNode.id === PlatformRoots.other)) {
...
function renderHistoryPanel() {
  ...
  filteredData.forEach((item) => {
    ...
    li.addEventListener('click', () => {
      ExtensionAPI.tabs.create({ url: item.url });
    });
...
async function showHistoryContextMenu(e, item) {
  ...
  addToBookmarkItem.addEventListener('click', async () => {
    ...
    let parentId = PlatformRoots.toolbar;
    if (folders.length > 0) {
...
async function renderShortcutsPanel() {
  ...
  shortcuts.forEach((shortcut, index) => {
    ...
    item.addEventListener('click', () => {
      ExtensionAPI.tabs.create({ url: shortcut.url });
    });
...
async function showAddShortcutModal() {
  ...
  modal.innerHTML = `
    <div class="edit-modal-content">
      <h3 class="edit-modal-title">${Utils.escapeHtml(String(I18n.t('common.addShortcut')))}</h3>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.shortcutTitle')))}</label>
        <input type="text" class="edit-modal-input" id="add-shortcut-title" value="${Utils.escapeHtml(String(currentTabInfo?.title || ''))}">
      </div>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.shortcutUrl')))}</label>
        <input type="text" class="edit-modal-input" id="add-shortcut-url" value="${Utils.escapeHtml(String(currentTabInfo?.url || ''))}">
      </div>
      <div class="edit-modal-buttons">
        <button class="edit-modal-btn edit-modal-cancel">${Utils.escapeHtml(String(I18n.t('common.cancel')))}</button>
        <button class="edit-modal-btn edit-modal-save">${Utils.escapeHtml(String(I18n.t('common.save')))}</button>
...
async function showEditShortcutModal(shortcut) {
  ...
  modal.innerHTML = `
    <div class="edit-modal-content">
      <h3 class="edit-modal-title">${Utils.escapeHtml(String(I18n.t('common.modifyShortcut') || '修改捷径'))}</h3>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.shortcutTitle')))}</label>
        <input type="text" class="edit-modal-input" id="edit-shortcut-title" value="${Utils.escapeHtml(String(shortcut.title))}">
      </div>
      <div class="edit-modal-field">
        <label class="edit-modal-label">${Utils.escapeHtml(String(I18n.t('common.shortcutUrl')))}</label>
        <input type="text" class="edit-modal-input" id="edit-shortcut-url" value="${Utils.escapeHtml(String(shortcut.url))}">
      </div>
      <div class="edit-modal-buttons">
        <button class="edit-modal-btn edit-modal-cancel">${Utils.escapeHtml(String(I18n.t('common.cancel')))}</button>
        <button class="edit-modal-btn edit-modal-save">${Utils.escapeHtml(String(I18n.t('common.save')))}</button>
...
async function loadLayoutSettings() {
  try {
    ...
    document.documentElement.style.setProperty('--shortcut-icon-size', `${settings.shortcutIconSize || 60}px`);
...
function setupEventListeners() {
  ...
  document.getElementById('btn-open-manager').addEventListener('click', () => {
    ExtensionAPI.tabs.create({
      url: ExtensionAPI.runtime.getURL('manager/manager.html')
    });
  });
  ...
  const debouncedReload = Utils.debounce(async () => { const {restore_job}=await Storage.get('restore_job'); if(restore_job?.status!=='running')await loadBookmarkTree(); }, 300);
  ExtensionAPI.bookmarks.onCreated.addListener(debouncedReload);
  ExtensionAPI.bookmarks.onRemoved.addListener(debouncedReload);
  ExtensionAPI.bookmarks.onChanged.addListener(debouncedReload);
  ExtensionAPI.bookmarks.onMoved.addListener(debouncedReload);
  
  // 监听设置变化，重新加载常用配置
  ExtensionAPI.storage.onChanged.addListener(async (changes, namespace) => {
    if (namespace === 'local' && changes.shortcuts) await renderShortcutsPanel();
    if (namespace === 'local' && changes.restore_job?.newValue?.status === 'complete') await loadBookmarkTree();
