let disposeVirtualList=null;
let loadedFolderId=null;
let viewRevision=0;
/**
 * 管理器脚本
 */
...
const state = {
  currentFolderId: PlatformRoots.toolbar,
  bookmarks: [],
  folders: [],
  selectedIds: new Set(),
  viewMode: 'list',
  ...
  draggedItem: null,
  draggedIndex: -1,
  expandedFolders: new Set([PlatformRoots.toolbar, PlatformRoots.other]) // 默认展开书签栏和其他书签
};

document.addEventListener('DOMContentLoaded', async () => {
  await platformReady;
  state.currentFolderId=PlatformRoots.toolbar; state.expandedFolders=new Set([PlatformRoots.toolbar,PlatformRoots.other]);
  // 初始化主题和语言
  ThemeManager.init();
  await I18n.init();
  ...
  await loadBookmarks(PlatformRoots.toolbar);
  setupEventListeners();
  ...
  ExtensionAPI.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === 'refreshBookmarks') {
      console.log('Refreshing bookmarks after restore...');
      loadFolderTree().then(() => {
        loadBookmarks(state.currentFolderId || PlatformRoots.root);
      });
    }
  });
});
...
async function loadBookmarks(folderId) {
  ...
  try {
    const [tree,tags]=await Promise.all([BookmarkUtils.getTree(),BookmarkTags.getAll()]);
    if(revision!==viewRevision) return;
    const byId=new Map(); const visit=n=>{byId.set(n.id,n);let count=n.url?1:0;for(const c of n.children||[])count+=visit(c);n.bookmarkCount=count;return count;};tree.forEach(visit);
    const folder=byId.get(folderId) || byId.get(PlatformRoots.toolbar);
    if(loadedFolderId!==folder.id)document.querySelector('.bookmark-container').scrollTop=0;
    loadedFolderId=folder.id;
    state.currentFolderId=folder.id;
    state.folders=(folder.children||[]).filter(n=>!n.url);
    state.bookmarks=(folder.children||[]).filter(n=>n.url).map(n=>({...n,tags:tags[n.id]||[]}));
    state.searching=false;
    renderBookmarks(); updateBreadcrumb(folder.id);
  }catch(error){console.error(error);}
}

async function loadFolderCounts() {
  const tree=await BookmarkUtils.getTree(); const counts=new Map();
  const visit=n=>{let count=n.url?1:0; for(const c of n.children||[]) count+=visit(c);counts.set(n.id,count);return count;};tree.forEach(visit);
  state.folders.forEach(f=>f.bookmarkCount=counts.get(f.id)||0);
}

async function countBookmarksInFolder(folderId) {
  const tree=await BookmarkUtils.getTree();let found;
  const find=n=>{if(n.id===folderId)found=n;(n.children||[]).forEach(find);};tree.forEach(find);
  const count=n=>n.url?1:(n.children||[]).reduce((sum,c)=>sum+count(c),0);
  return found?count(found):0;
}
...
function renderBookmarks() {
  disposeVirtualList?.(); disposeVirtualList=null;
  ...
  if(state.viewMode==='list'&&folders.length+bookmarks.length>300) {
    const items=[...folders,...bookmarks];
    disposeVirtualList=VirtualList.mount(listEl,document.querySelector('.bookmark-container'),items,(item,index)=>item.url?createBookmarkRow(item,index):createFolderRow(item,index),()=>Boolean(state.draggedItem));
    updateViewDisplay();return;
  }
  // 先渲染文件夹
  folders.forEach((folder, index) => {
    if(state.viewMode==='list') listEl.appendChild(createFolderRow(folder, index));
    if(state.viewMode==='grid') gridEl.appendChild(createFolderCard(folder, index));
  });

  // 再渲染书签
  bookmarks.forEach((bookmark, index) => {
    if(state.viewMode==='list') listEl.appendChild(createBookmarkRow(bookmark, folders.length + index));
    if(state.viewMode==='grid') gridEl.appendChild(createBookmarkCard(bookmark, folders.length + index));
  });
...
function createBookmarkRow(bookmark, index) {
  ...
  title.textContent = bookmark.title || I18n.t('common.noName');
...
function createFolderRow(folder, index) {
  ...
  const title = document.createElement('div');
  title.className = 'bookmark-title';
  title.textContent = folder.title || I18n.t('common.noNameFolder');

  const count = document.createElement('div');
  count.className = 'bookmark-url';
  count.textContent = `${folder.bookmarkCount || 0} ${I18n.t('common.items')}`;
...
function renderFolderDetailPanel(folder) {
  ...
  panel.innerHTML = `
    <div class="detail-header" data-i18n="common.folderDetail">文件夹详情</div>
    <div class="detail-content">
      <div class="detail-field">
        <div class="detail-label" data-i18n="common.name">名称</div>
        <input type="text" class="detail-input" id="detail-title" value="${Utils.escapeHtml(folder.title || '')}">
      </div>
      <div class="detail-field">
        <div class="detail-label" data-i18n="common.contains">包含内容</div>
        <div class="detail-value">${Utils.escapeHtml(String(folderCount))} ${Utils.escapeHtml(String(I18n.t('common.folders')))}，${Utils.escapeHtml(String(bookmarkCount))} ${Utils.escapeHtml(String(I18n.t('common.bookmarks')))}</div>
      </div>
      <div class="detail-field">
        <div class="detail-label" data-i18n="common.addTime">添加时间</div>
        <div class="detail-value">${Utils.escapeHtml(String(folder.dateAdded ? Utils.formatDate(folder.dateAdded) : I18n.t('common.unknown')))}</div>
      </div>
      <div class="detail-actions">
        <button class="btn" id="open-folder-btn" data-i18n="common.enterFolder">进入文件夹</button>
        <button class="btn btn-primary" id="save-folder-btn" data-i18n="common.save">保存</button>
      </div>
    </div>
  `;
...
async function deleteFolder(folderId, folderTitle) {
  const confirmed = await showConfirm(I18n.t('confirm.deleteFolder', { name: folderTitle }), { danger: true });
  if (!confirmed) return;
  ...
  try {
    ...
    await showAlert(I18n.t('common.deleteFailed'));
  }
}
...
async function renderDetailPanel(bookmark) {
  ...
  panel.innerHTML = `
    <div class="detail-header" data-i18n="common.bookmarkDetail">书签详情</div>
    <div class="detail-content">
      <div class="detail-field">
        <div class="detail-label" data-i18n="common.title">标题</div>
        <input type="text" class="detail-input" id="detail-title" value="${Utils.escapeHtml(bookmark.title || '')}">
      </div>
      <div class="detail-field">
        <div class="detail-label" data-i18n="common.url">网址</div>
        <input type="text" class="detail-input" id="detail-url" value="${Utils.escapeHtml(bookmark.url || '')}">
      </div>
      <div class="detail-field">
        <div class="detail-label" data-i18n="common.tags">标签</div>
        <div class="detail-tags-input">
          <input type="text" class="detail-input" id="detail-tag-input" data-i18n-placeholder="common.enterTag" placeholder="输入标签名后按回车添加">
        </div>
        <div class="detail-tags" id="detail-tags-container"></div>
      </div>
      <div class="detail-field">
        <div class="detail-label" data-i18n="common.quickAddTag">快速添加标签</div>
        <div id="tag-selector-container" class="tag-selector-container"></div>
      </div>
      <div class="detail-field">
        <div class="detail-label" data-i18n="common.addTime">添加时间</div>
        <div class="detail-value">${Utils.escapeHtml(String(bookmark.dateAdded ? Utils.formatDate(bookmark.dateAdded) : I18n.t('common.unknown')))}</div>
      </div>
      <div class="detail-actions">
        <button class="btn" id="open-bookmark-btn" data-i18n="common.open">打开</button>
        <button class="btn btn-primary" id="save-bookmark-btn" data-i18n="common.save">保存</button>
      </div>
    </div>
  `;
...
async function addTagToBookmark(bookmarkId,tagName) {
  await BookmarkTags.addTag(bookmarkId,tagName); const tags=await BookmarkTags.getTags(bookmarkId);
  renderDetailTags(bookmarkId,tags);const bookmark=state.bookmarks.find(b=>b.id===bookmarkId);if(bookmark)bookmark.tags=tags;renderBookmarks();
}

async function removeTagFromBookmark(bookmarkId,tagName) {
  await BookmarkTags.removeTag(bookmarkId,tagName);const tags=await BookmarkTags.getTags(bookmarkId);
  renderDetailTags(bookmarkId,tags);const bookmark=state.bookmarks.find(b=>b.id===bookmarkId);if(bookmark)bookmark.tags=tags;renderBookmarks();
}
...
async function deleteBookmark(id) {
  try {
    ...
    await showAlert(I18n.t('common.deleteFailed') + ': ' + (error.message || I18n.t('common.unknown')));
  }
}
...
async function batchDelete() {
  ...
  const message = hasFolder
    ? I18n.t('confirm.deleteItemsWithFolders', { count: count })
    : I18n.t('confirm.deleteBookmarks', { count: count });

  const confirmed = await showConfirm(message, { danger: true });
  if (!confirmed) return;
...
async function handleSearch(e) {
  ...
  if(!query){await loadBookmarks(state.currentFolderId);return;}
  const revision=++viewRevision;
  const [tree,tags]=await Promise.all([BookmarkUtils.getTree(),BookmarkTags.getAll()]);
  if(revision!==viewRevision)return;
  let folder;const find=n=>{if(n.id===state.currentFolderId)folder=n;(n.children||[]).forEach(find);};tree.forEach(find);
  const results=[];const tagQuery=query.replace(/^#/,'');
  const visit=n=>{const nodeTags=tags[n.id]||[];const textMatch=!query.startsWith('#')&&((n.title||'').toLowerCase().includes(query)||(n.url||'').toLowerCase().includes(query));if(textMatch||nodeTags.some(t=>t.toLowerCase().includes(tagQuery)))results.push({...n,tags:nodeTags});(n.children||[]).forEach(visit);};
  (folder?.children||[]).forEach(visit);
  state.folders=results.filter(n=>!n.url);state.bookmarks=results.filter(n=>n.url);state.searching=true;renderBookmarks();
}
...
async function addNewBookmark() {
  ...
  if (!title || !url) {
    await showAlert(I18n.t('common.enterTitleAndUrl'));
    return;
  }
...
function hideNewFolderModal() {
  document.getElementById('new-folder-modal').classList.remove('visible');
}

async function createNewFolder() {
  const title = document.getElementById('new-folder-name-input').value.trim();
  if (!title) {
  ...
  try {
    ...
    await showAlert(I18n.t('common.createFolderFailed'));
  }
}

const refreshManager=Utils.debounce(()=>handleBookmarkChanged().catch(console.error),150);
function setupEventListeners() {
  ...
  document.getElementById('new-folder-modal').addEventListener('click', (e) => {
    if (e.target.id === 'new-folder-modal') hideNewFolderModal();
  });
  ...
  document.getElementById('new-folder-name-input').addEventListener('keydown', (e) => {
  ...
  ExtensionAPI.bookmarks.onCreated.addListener(refreshManager);
  ExtensionAPI.bookmarks.onRemoved.addListener(refreshManager);
  ExtensionAPI.bookmarks.onChanged.addListener(refreshManager);
  ExtensionAPI.bookmarks.onMoved.addListener(refreshManager);
}

// 处理书签变化
async function handleBookmarkChanged() {
  const {restore_job}=await Storage.get('restore_job');
  if(restore_job?.status==='running')return;
  await loadFolderTree();
  const input=document.getElementById('search-input');
  if(input.value.trim())await handleSearch({target:input});else await loadBookmarks(state.currentFolderId);
}

// 拖拽排序功能
function handleDragStart(e) {
  if(state.searching){e.preventDefault();return;}
...
async function moveBookmark(fromIndex,toIndex) {
  if(state.searching||fromIndex===toIndex)return;
  const display=[...state.folders,...state.bookmarks];const moving=display[fromIndex];if(!moving)return;
  const rest=display.filter(n=>n.id!==moving.id);const anchor=rest[toIndex];
  const siblings=(await BookmarkUtils.getChildren(state.currentFolderId)).filter(n=>n.id!==moving.id);
  const index=anchor?siblings.findIndex(n=>n.id===anchor.id):siblings.length;
  if(index<0)return;
  await BookmarkUtils.move(moving.id,{parentId:state.currentFolderId,index});
  await loadBookmarks(state.currentFolderId);
}
...
function toggleBatchTag(tag, element) {
  ...
  if (selectedBatchTags.has(tag)) {
    ...
    // 从 tag-cloud 中移除
    const tagEl = tagCloud.querySelector(`[data-tag="${CSS.escape(tag)}"]`);
...
async function addTagsToSelected() {
  ...
  // 为每个选中的书签添加标签
  await BookmarkTags.batchAdd([...state.selectedIds],allNewTags);
...
ExtensionAPI.storage.onChanged.addListener((changes,area)=>{
 ...
 if(changes.bookmark_tags||changes.tagGroups||changes.restore_job?.newValue?.status==='complete')refreshManager();
 if(changes.layoutSettings){FaviconService.initSize().then(()=>renderBookmarks());}
});
